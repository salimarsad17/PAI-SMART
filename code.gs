 ...
 * 4. Hapus semua kode default di file Code.gs, lalu SALIN & TEMPEL (PASTE) seluruh kode di file ini.
 ...
 *    - Klik akun Google Anda > klik "Lanjutan" (Advanced) > klik "Buka ... (tidak aman)" / "Go to ... (unsafe)".
 *    - Klik "Izinkan" (Allow).
 ...
 *    - Yang memiliki akses (Who has access): "Siapa saja" (Anyone). -> INI SANGAT PENTING!
...
function setupSpreadsheet() {
  ...
  // Hapus 'Sheet1' / 'Sheet 1' default jika masih kosong
 ...
 * Bisa digunakan untuk testing di browser dan pengambilan data cepat
 */
function doGet(e) {
 ...
 * Digunakan oleh Web App untuk operasi sync, tambah, ubah, dan hapus data
 */
function doPost(e) {
  var action = 'ping';
...
function handleApiRequest(action, data, params) {
  try {
    switch (action) {
      ...
      // 11. BULK SYNC / FULL EXPORT (Sinkronisasi Seluruh Data Sekaligus)
      case 'syncAll':
...
function getSheetData(sheetName) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    for (var j = 0; j < headers.length; j++) {
      ...
      // Auto-parse jika cellValue berupa JSON string (seperti array badges atau options)
      if (typeof cellValue === 'string' && (cellValue.indexOf('[') === 0 || cellValue.indexOf('{') === 0)) {
        try {
          cellValue = JSON.parse(cellValue);
        } catch (e) {
          // Tetap string jika bukan valid JSON
        }
      }
...
function saveOrUpdateRecord(sheetName, idField, recordData) {
    ...
    if (idColIndex > 0) {
      ...
      for (var r = 0; r < idValues.length; r++) {
        if (String(idValues[r][0]) === String(recordData[idField])) {
          targetRowIndex = r + 2; // +2 karena indeks 0 di baris ke-2
          break;
        }
      }
    }
  ...
  // Siapkan baris data sesuai urutan header
  var rowValues = [];
